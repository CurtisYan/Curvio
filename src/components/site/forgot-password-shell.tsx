    ...
    <main className="container-narrow flex min-h-screen items-center justify-center pt-24 pb-20">
      <Card className="w-full max-w-md space-y-6">
        <div>
          ...
          <p className="mt-3 text-sm leading-6 text-muted">Enter the email you used to register and we'll send a reset link.</p>
        </div>
        {error ? (
          <div className="rounded-lg border border-error/20 bg-error/5 px-3 py-2 text-sm text-error">{error}</div>
        ) : null}
        {sent ? (
          <div className="rounded-lg border border-primary/20 bg-primary/5 px-3 py-2 text-sm text-primary">Link sent.</div>
        ) : null}
